...
| [**Allods Online**](https://github.com/vs-sr-dev/pc-allodsonline-doc) | 2009 *(the year the game shipped in Russia — and the first row in this column where **five candidate dates** had to be argued against each other, with the filesystem eliminated first. There are 281 distinct mtimes on **four days**, 254 of them the day of the download, and the oldest byte-timestamp in seventeen gigabytes belongs to a **Microsoft DLL** inside the launcher's browser. The *Tesla Effect* row said its filesystem dated nothing because Steam had rewritten every mtime into one window; this one is the same finding from the other side — the clock dates the **download**, 2026-09-01 07:59:58 to 08:06:25 UTC, 387 seconds for 17 GB. Inside the archives, **99.0590 % of 355,159 entries declare `2009-10-02 00:00:20`** and there are only **five distinct timestamps in the whole population** — it is a build constant, not a date, and the proof is `Bin/pack.loc`, stamped 2009 and containing the strings `2020 - "Русь" (Russia)` and `2023 (subscription servers) — Золотая Орда (RU)`. The real build event survives in **exactly two entries of 355,159**: `Version/DataPacksInfo.bin` and `Version/GameCenterInfo.bin`, `2026-08-13 17:48:28` and `:26`, two seconds apart, with the two `AOgame.exe` linked four hours earlier the same afternoon and the torrent cut twenty-seven minutes later. So: **2009** the game, **2026-08-13** this build, **2026-09-01** this copy — and the cell takes the game's year, as the *Final Fantasy VIII* row did and for the same reason. The *Meridian 59* row's thirty years of operational layers is the nearest precedent here and it is a live service too)* | **Astrum Nival / Mail.Ru, today MY.GAMES** *(and the object was asked before the cell was filled, as the *Plundered Hearts* row asks rather than assumes. Outside the archives, `MY.GAMES` occurs **1,624 times in 26 files** and `Nival` **16 times in 5** — a hundred to one for the operator over the developer. The full lineage is legible in four `CompanyName` fields in one directory: **Nival Interactive** on the modding tools, **Nival Online** on `protect.dll` **5.70.009.0** whose product string still reads `Allods Online (for ALLODS_2 test)`, **ASTRUM LAB LLC** on `protect.dll` **6.02.2605.072** called `Allods Online New`, and **MY.GAMES B.V.** on nine images of `MY.GAMES Anti-Cheat 4.48.0`. Two generations of the same protection module ship at once. And the two files that matter most — `bin/AOgame.exe` and `bin_x64/AOgame.exe` — **declare no company at all**: 54 of the object's 108 PE images are anonymous, including the game)* |   | **Seventeen gigabytes that never touched physical media, and the first object in this list that could be *verified* rather than believed.** **1,186 files, 17,366,372,506 bytes, 281 mtimes on four days, 35 extensions** — a live client of a live service, delivered because **Steam hands over a downloader**: app 459160 installs a 13 MB loader that starts the publisher's launcher, which embeds **Chromium 99** and a BitTorrent client and fetches 16.5 GB from `p2p.gc.my.games`, so the game is not on Steam and Steam's content system never touched a byte of it. **It is also the first object here that changed while it was being described**: the session briefing measured 17,366,370,830 bytes and the census run nineteen minutes later measured **1,676 more**, all of it in files the launcher writes, and **sixteen files could not be hashed at all** because a running process held them open. **The oldest question in this list gets its eleventh answer and its first recomputable one.** The object carries **seven** self-declarations and **five were checked here**: a 284,562-byte `.torrent` with **8,666 SHA-1 digests of 2 MiB each**, a second 10,155-byte torrent from 2022 with **482 more**, a signed internal manifest with **871 lengths and 871 MD5s**, 355,159 ZIP central-directory entries, a `pack.loc` footer declaring its own string blob, **1,241 `.bsb` length words** and **1,250 of 4,029 sampled `.bin`**. The results: **8,666 of 8,666 pieces in 37.8 seconds, 482 of 482, 871 of 871 lengths, 871 of 871 hashes, 0 failed, 0 missing, 0 short** — **98.3639 % of the object covered by a verified digest**, which no previous session could write. The verification required rebuilding the byte stream the descriptor names rather than the one on disk, because **875 of the torrent's 1,750 entries are BEP 47 padding** synthesised to align every real file to a piece boundary: 1,154,600,016 bytes that are counted, hashed, and never written. **Three totals of the same thing close into one identity with a difference of zero**: `18,173,919,232 declared − 1,154,600,016 padding + 347,053,290 not-in-the-torrent = 17,366,372,506 on disk`; and the internal manifest's 17,019,211,984 plus **the four files it structurally cannot list** — itself, the icon and the installer's two — comes to 17,019,319,216, the torrent's real payload, **also to the byte**. The revision it forces on that question is the sharpest one it has had: the descriptor is a property of **what can go wrong between the maker and the reader**. On a disc that is *where does this chunk end*, and a length suffices; over a network it is *are these the right bytes*, and a length is useless — so the mechanism changed shape from a number you believe to 8,666 hashes you can check. **The container is 693 ZIP files that compress nothing**: 355,157 stored entries against **2** deflated, and an ownership map built by seeking to all 355,159 local headers — because the central directory does not declare their size and an estimate is not a measurement — that **closes exactly: 0 bytes owned by nobody, 0 owned by two, over 16,496,013,793**. Sixth exact closure in a row and the first over sixteen gigabytes; structure costs **0.469576 %**, 218.06 bytes an entry, and the reason is the names — mean path **70.62 characters**, stored twice. **Zero ZIP64 records, zero non-ASCII entry names, zero duplicate paths within any archive**, and **351,715 distinct paths of 355,159**: the archives are 99.03 % internally unique and every one of the 3,444 repeats is a `_x86` name appearing once more under `_x64`. **The redundancy is a product decision and the naming nearly hid it.** `.Mini.` and `.HiRes.` share **zero** entry names as spelt — a literal set intersection returns nothing and a reader who stops there concludes the trees are unrelated — because the high-resolution archives spell the same path `X.(Texture).hi.bin`. Remove the infix and **71,587 names are shared**, the HiRes copy is larger on **71,586 of them (99.9986 %)** at a ratio of **11.87**, `.Mini.` is the complete 201,199-entry set and `.HiRes.` is an overlay plus 1,130 assets that exist in no other form. The `_x86`/`_x64` pair is a different question with a better answer: same 3,444 names, **90.9 % identical in size**, and the four archives are the same data serialised for two pointer widths — which the object then proves in its own text, because **612 of `pack.loc`'s 264,463 strings differ between the two builds**, every one of them a machine word written into the localisation as hexadecimal and padded to the host pointer size (`href="42360000"` against `href="4236000000000000"`). **A localisation table should not know how wide a pointer is; this one does, 612 times.** **Q21 gets its cleanest data point**: `crossbin.py` on the two `AOgame.exe`, truncated to a declared 4 MB and the truncation stated, finds **5,204 shared bytes = 0.1301 %**, longest run 3,161 and it is a sparse table — **two builds of one source share two-fifths of what two different 1993 games shared**, because instruction encoding changes completely between architectures and what survives is what a compiler did not write. **Eighty per cent of the archives is one extension and it was not opened.** `.bin` is 350,470 entries and 13,100,719,400 bytes; **4,029 were inflated, 1.1496 %**, at a fixed deterministic stride, and the count *of 350,470* is printed on the line of every class in the taxonomy — because 350,470 files with descriptive English names are an invitation to write a taxonomy from the names. Doing it properly separates **what the name claims** from **what the first eight bytes say** and cross-tabulates them: `SkeletalAnimation`, `Geometry`, `ParticleAnimation` and `UITexture` are internally uniform over 1,375 opened files, `CollisionMesh` uses the leading word differently on all 94, and **`Texture` is not one thing** — ten distinct leading values across 1,784 files, which no filename could reveal. **1,250 of 4,029 declare their own length in the second word**, and three of the classes do it on **100 %** of the files opened. The one place a name *was* trusted produced the session's own worst error: `Maps/<zone>/<NNN_NNN>/` block coordinates were divided by a bounding box in units of 1, giving a fullest-zone figure of **2.60 %** and a story about a scattered world. The coordinates advance in steps of ten. Deriving the step per zone as a gcd: **mean fill 90.01 %, 270 of 306 zones over 60 %, and 239 of 306 exactly 100 %** — a wrong answer that was **35 times too small**, and the correction is in the tool's docstring. **The text is 2,877,631 words** over 181,513 distinct strings in a 264,463-record table whose format is published nowhere and was derived from the bytes: magic `Az0h`, a word count, `(length, offset)` pairs, UTF-16LE strings NUL-terminated and 4-byte aligned — forced by arithmetic, not assumed, since `16 + (9+1)×2 = 36` is the next offset and a reader who assumes byte lengths fails at the third record — and then a **footer declaring the byte length of the string blob**, from which the base is `len(file) − declared`, exactly, in both variants. **264,463 of 264,463 records land on a NUL terminator; two bytes of 39,236,988 are never reached.** **And the Russian that survives an `eng_eu` build is eighty-five strings of 181,513, of which forty-eight are typing mistakes.** A Cyrillic `С` is drawn identically to a Latin `C`, and `х` to `x`: `Сolonel of the Imperial Keepers`, `Т92 Arty`, `Backstop МК9`, `Open 3х3 Combat Rating`, and — the best line in seventeen gigabytes — **one tooltip whose consecutive list items read `<o id="740032689"/> х 5` and `<o id="167407623"/> x 5`, the Cyrillic multiplication sign and the Latin one, one after the other**. Forty of the forty-eight are that `х`. They are not localisation failures; they are the fingerprints of the people who typed the English, and no spell-checker flags a character that looks right. The thirty-seven that are genuinely Russian are **systematic and mostly untranslatable**: hall-of-fame lists of Russian server players with half-translated class headings (`Bards:`, `Warriors:`, `Жрецы:`), a Latin-to-Cyrillic transliteration table shipped as data, a search-prefix index that spells out `u / un / uni / … / universe` and `в / вс / все / … / вселенная` in one string, emote pairs, and **one class-change dialogue box that was simply missed**. **The largest body of Russian is not in the game text at all**: the client ships its modding API in source — 74 readable files, five sample addons — and beside them `ModdingDocuments.zip`, **3,100 files and 25,649,521 bytes containing 3,031 HTML pages of which 2,992 are Lua API reference, and 3,031 of 3,031 contain Cyrillic**, 180,365 Russian words. The player-facing text was translated to the last string but one; the developer-facing manual was not translated at all. Beside it the compiled side: **3,240 LuaJIT files, all 3,240 walked to their last byte**, 36,948 prototypes, **396,460 constant strings, 0 of them Cyrillic, 0 invalid UTF-8**, and a version byte of **161** where LuaJIT writes 1 or 2, which is recorded and not explained. **The sound is 1,276 FMOD banks and 1,241 of them were hiding.** The briefing named 35 `.fsb`; twice as much again is in **1,241 `.bsb`, which are a zlib stream wrapping a 32-bit length and an FSB5 bank** — and that length is correct on **1,241 of 1,241, difference zero**, the smallest volume descriptor in this list. **All 1,276 banks close their declared sizes exactly**, 31,642 samples, **37 h 14 m 51 s — 29 h 05 m 35 s once the eight hours of English voice shipped twice under two pointer widths are counted once** — and **not one audio frame was decoded**: every duration is `samples / rate` from a 64-bit header, and both of the reader's bit layouts were settled by the object rather than from memory, one because the wrong ordering overran a bank's declared 7,628 bytes by a factor of **189**, the other because the usually-quoted 32-byte offset unit put the last sample of **145 of the first 150 banks** past the end of its own data while 16 puts **0 of 150** outside. Voice **28.03 %**, creatures **26.30 %**, music 21.31 %, and **footsteps 0.34 %** — six minutes of foley in a world where a handful of samples are played ten million times. **The film is 44 Ogg Theora files, 24 m 41 s at 9,661 kbit/s**, measured from page headers and granule positions with **zero frames decoded**, `KFGSHIFT` read rather than assumed, and the seek-to-the-end shortcut validated against a full page walk that consumed its file to the last byte first. **The thesis column was put under pressure and was not widened.** Counted as it always has been, recorded reality here is **17.4595 %** — a fall of seventy points from the 88.11 % of the row above — and the obvious rescue is to redefine the column as *content authored outside the code*, which would score this object 94.72 %. The document refuses, because under that reading **every object in this list scores 90–100 %**, a 1986 Infocom story file included, and a column on which a Z-machine game and an MMORPG both score 99 % has stopped separating anything. A **second** column is added instead — **calculated world, 75.4373 %** — and the two together say the thing neither says alone: in 1994 a CD-ROM spent 88 % of itself on two and a half hours of recorded voice, and in 2026 a client spends 17 % on twenty-nine hours of sound and twenty-five minutes of film and **75 % on a place that does not exist**. **Absolute paths: 2,002 drive-lettered, 0 source-tree**, in 60 files from 21 build roots — second-highest here, and unlike *Final Fantasy VIII*'s 31,737 these are not one debug build but twenty-one machines. The briefing expected 313 in four third-party DLLs; **`WebCore.dll` alone carries 852**, and the population the briefing did not have is the publisher's own: **491 paths on two drives carrying one developer's surname**, one of which reads `D:\DKolosov\Professional\mail.ru\trunk\a1\fmodexsrc_4.44.54wc\…` **inside a DLL whose version resource says `Firelight Technologies`** — a `CompanyName` says who wrote a library, a build path says who compiled it, and here they are different people. And the game's own executables leak **exactly one line each**: `E:\BuildAgent\work.custom\BranchBuilds_RC6\code\client\FinalRelease\AOgame.pdb`. **Thirteen layers, difference from the census 0, no row marked unknown** — and one of them is a layer no published client has. **This object contains a person, and the person is whoever downloaded it.** 210 files and 19,657,422 bytes were written by the machine it is installed on: a complete Chromium profile with a cookie database, `Local Storage`, `Code Cache` and an **IndexedDB store for `youtube.com`**, a torrent session with a `save_path`, and a launcher log carrying a **SteamID64 five times**. Those bytes are counted — 202 files, 0.1100 % — and **not read**: no tool in the repository opens that directory, the identifier and the operator's **360 absolute paths in 13 files** were counted by commands whose only output was the count and appear nowhere whole, truncated or masked, `torrentx.py` prints `save_path` as a length and refuses its value, and even the **186 SHA-1 digests** of those files were replaced with `withheld` in the committed census, because a digest is a one-way function *of* somebody's browsing. The operating system agreed twice over: sixteen of them were locked by a running process anyway. **Leftovers: candidates 13,129,211,088 bytes, of which 87,193,778 = 0.5021 % survive the argument** — fifty-one Chromium locale files for languages the launcher does not offer, a 62,927,054-byte 7-Zip of a Chromium that is already unpacked in the next directory, two empty text files and **one 768-byte archive holding one sound bank of one sample and zero seconds**. The 60.8 % that is duplicate *by design* is argued out row by row. **And what was never opened is a headline, not a footnote: 13,043,642,343 bytes = 75.1086 % never decompressed, 94.1924 % never decoded**, with the six `.bin` family readers, the 7-Zip decoder, the 759-byte `ROX` format and the LuaJIT disassembler that would be needed to go further named exactly. **Seventy predictions written before a piece was hashed, scored at 72.9 % — method 89.3 %, content 68.8 %** — and the most useful clause in the file is a **miss**: it predicted the thesis column would be redefined to make this object look good, and the chapter refused. **The Saga cell is empty**, for the fourth row running and for a reason this object states more cleanly than any of them: *Allods* is a real series — *Rage of Mages* I and II, *Evil Islands* — and this index fills that column on **measured shared bytes**. There is no second *Allods* object on this machine to measure against, and this one is the least fillable of the four, because a 2026 client of a live service shares nothing measurable with a 1998 CD-ROM even in principle: not a container, not a codec, not a byte. The three rows above declined the cell on the *absence* of a second object; this one declines it on the **impossibility of the comparison** |
| [**CLIC 02/97**](https://github.com/vs-sr-dev/pc-clic0297-doc) | 1997, to the second — the primary volume descriptor says `1997-02-14 18:54:32.00 GMT+0` and writes it twice, identically. *(Which **issue** it is, is the harder half, and this is the first row where the object contradicts its own catalogue: the volume identifier is `CLIC`, with no number and no date, unlike CLIC 11's `CLIC_11`; the only directory naming an issue is `NUMERI/CLIC297/`, February 1997; and `/LEGGIMI.TXT` at the root opens **«Clic! Cd-rom di Marzo»** and then says it contains the February website. It is the **March** disc carrying **February** content, and it advertises an April trade fair. Called **02/97** here after the directory, and after this collection's habit of sorting on what the filesystem says, with the readme's disagreement recorded rather than resolved. The back-issue archive it carries is coded **4, 6, 7, 9, 10, 11, 12** and then `197`, `297` — and those codes are **months, not positions in a sequence**: the magazine cites its own back issues only by month name, five times (*«numero di ottobre»*, *«di maggio»*, *«di giugno»*…), `PAG/12CO003.HTM` names the October issue as a back number, and `PAG/11CO005.HTM` previews a release *«Il 15 novembre»* in the future tense. So 4–12 are April to December 1996 and the year was appended only when 1996 ran out. **There is no running issue count for `CLIC_11` to be the eleventh of**, which dates the other object in this index: a disc cut on 1997-10-20 is the **November 1997** issue, as this one cut on 14 February is the March one)* | **CLIC!**, published by **Mondadori Informatica S.p.A.**, CD-ROM produced by **GLAMM Interactive s.r.l.** of Milan — and the three are named in inverse proportion to what they made. `/CREDITS.HTM` is a published colophon crediting thirteen people, and **both names under *Produzione CD-ROM*** are GLAMM's. Counted as literal byte strings across all 2,818 files, the magazine names itself **1,649 times** (`Clic!` 1,104 + `CLIC` 545) and contributed **0.82 %** of the bytes; **Mondadori** appears 86 times; **Leader**, whose catalogue is 72.4342 % of the disc, appears **69**; **GLAMM**, who built the object, appears **fourteen**. Exactly one binary on the disc carries a `CompanyName` of `GLAMM Interactive` — `SETUPIE.EXE`, 16,896 bytes, PE32, COFF-stamped 1997-02-12 14:16:04 — and it is what `AUTORUN.INF` runs | CLIC — **and this is the first Saga cell in this index to fill on measured *process* rather than on shared payload, which is a change to how the rule is applied and is argued out in [chapter 14](https://github.com/vs-sr-dev/pc-clic0297-doc/blob/master/docs/14-against-clic11.md) rather than assumed.** The two CLIC discs share **1,792,592 bytes and none of it is theirs**: `QTINSTAL.EXE` (Apple) and `VBRUN300.DLL` (Microsoft), plus a third matching hash that is two empty files and is not a crossing. **Zero bytes of either magazine's own work appears on both.** Filling the cell on a QuickTime installer would put every hybrid in this collection in a saga with every other, so the rule's implicit clause is made explicit — *shared bytes must belong to the objects, not to a third party both licensed* — and by that reading the payload measurement is **zero**. What fills the cell instead is five independent fingerprints of one production line: the same mastering program in two versions (`TOAST … 1993-1995 MILES SOFTWARE ENGINEERING` / `TOAST 2.5 Partition` here against `TOAST … 1997 ADAPTEC` / `Toast 3.5.2 PPC HFS Optimizer` there), the same ISO+HFS hybrid with an Apple partition map at block 0, the same empty implementor fields, **99.0622 % even seconds against 99.11 %**, and **two unowned sectors, both zero**, on two discs of different sizes mastered by different versions of one program | **Not a game and not a work — a magazine's cover disc, and the first object here measured from an image rather than from a drive.** 509,257,728 bytes, sha1 `ee3c2b6f7e16a178d4dd9093cdfe44ae007efca9`, cut on a Macintosh on St Valentine's Day 1997, and the thirteenth object in this collection. The **file count is a property of the walker and there are four**: 2,879 ISO directory records, 2,823 file records, 2,820 HFS catalogue files, **2,818 a PC can open** — and extraction proves the last without being asked, writing 2,823 records and leaving 2,818 files, because each of five Associated-File resource forks is overwritten by the data file sharing its name. **The centrepiece is an ownership map walked from both catalogues at once**, which nothing inherited could do: every previous sector map here assigns each sector one owner, and on a hybrid that is the wrong data structure — **96.7861 % of this disc is owned twice**, because Toast wrote one copy of every file and pointed an ISO tree and an HFS catalogue at the same bytes. Walked from both, the map runs **contiguously** from LBA 0 to LBA 248,358 — 16 system + 2 descriptors + 2 path tables + 116 ISO directories + 2 nobody + 5 HFS volume header + 510 extents B-tree + 510 catalogue B-tree + 247,195 file data + 1 alternate MDB = **248,359, the declared volume space** — and leaves **two sectors belonging to nobody**, both all-zero, in the gap the Apple partition map opens by starting the HFS volume at LBA **138.25**; both zero-byte files on the disc point their extent field at one of them. CLIC 11 also left two of 322,926, also zero. The 7,846 sectors the ISO side alone cannot account for decompose exactly, and **86.90 % of them are allocation-block padding**: an HFS block here is 10,240 bytes and an ISO extent 2,048, so every file must begin on a five-sector grid, and **2,821 of 2,823 records do** — the two exceptions being the zero-byte files, which makes it 100 % of everything containing a byte. The HFS side closes to the block: 49,439 file + 102 catalogue + 102 extents = **49,643 = `drNmAlBlks`, with `drFreeBks` 0**. **The padding is not empty, and that is the strangest thing on the disc**: not one of the 6,763 padding sectors is zero, **none repeats its own file's tail (0 of 2,395)**, and of 39 probes searched against the whole image 8 land inside another file, 14 only inside other padding, and **17 appear nowhere at all** — one of them a fax cover-sheet template that is on no file here. The inherited `slack.py`, measuring a different region for a different reason, agrees: **2,820 of 2,821 files have a dirty last sector**. Roughly **16.5 MB of this object is the uncleared working memory of a machine in Milan**. **The two catalogues differ by exactly five Finder objects and by no content at all** — `Desktop DB` (40,960 B), `Desktop DF` (**zero bytes and seven allocation blocks**), `Desktop Folder`, `Temporary Items`, `Trash` — which closes CLIC 11's Q6 in the negative and *structurally*: this hybrid has no Macintosh side to cross, being a Macintosh **view** of a PC disc rather than the 26,607,777 bytes of Mac-only payload the other shipped. **Five HTML files carry a 670-byte resource fork and it is somebody's cursor**: creator `R*ch`, one resource of type `BBSR` — BBEdit — holding a window rectangle (502 × 346, thirty pixels from the left), the font `Monaco` at size 9, and an insertion point, byte 757 of 1,135 for `CATAL.HTM`. They are not the tree's entry points; they are **the last five files anybody touched**, and `CATAL.HTM` carries the newest record on the disc, 17:03:32 — eight minutes before the HFS volume closed and 1 h 42 m 55 s before the ISO descriptor was written, a window in which **zero** directory records fall. **99.0622 % of the 2,879 records have an even seconds field**, MS-DOS's two-second grid fossilised on a disc a Macintosh program wrote, and all 52 records dated before 1993 — fifteen of them 1979 — live in **four directories belonging to two strata** while fifty-four other directories are clean. **Nine bodies of software and nobody in charge**: two versions of one authoring runtime ship in two folders because two suppliers each brought their own, two copies of DOS/4GW sit eleven directories apart, and of 75 executables exactly **two are 32-bit** — Microsoft's and Apple's. **72.4342 % of the disc is a mail-order shop's catalogue**: 221 products in three dBase tables of fourteen fields, including both `PRPUB` and `PRRIV` — the retail price *and the trade price*, 39,900–149,000 against 25,700–111,800 lire, a **33.76 % margin published by accident** — and **58.77 % of the entire object is 26 video trailers for other companies' games**. **Tomb Raider is 9.74 %**: all **16 of 16** retail level names, **2 of 4** cut scenes, `CURRENT.PHD` byte-identical to `TITLE.PHD`, no `.RPL` FMV and no Red Book music because there is one data track — and it is **the only one of the 221 catalogue products whose `DEMO` field is set**, product 100742, Eidos, 99,900 lire, so the catalogue filling three quarters of this disc advertises exactly one playable demo and it is sitting two directories away. **The disc kept its own build log, twelve times**: `WS_FTP.LOG` survives in twelve directories, 884 transfers, reconstructing the week from the back issues on Monday 10 February at 12:05 to `ville.jpg` and `orsay.jpg` at 13:59 the next day; Tomb Raider's folder kept a **Watcom linker map** naming every source file of the game and the stamp `96/09/03 22:38:51`, because a developer's own two-line patch script shipped beside it and copied it there; and `MAGDEMO/PICS/` kept a **Paint Shop Pro thumbnail cache**, recording that somebody opened a folder of pictures in 1996 to choose images. **1,281 absolute paths**, 857 distinct, in 81 files, from four vendors' machines in Derby, Bellevue and Milan — the most of any object here that is not a modern game, and 4.6× CLIC 11 — while the same tool's 248 *Macintosh-shaped* hits are **noise out of Cinepak data** and are reported as a property of the heuristic rather than added to a total. **Copy protection: none** — 12 markers over 75 executables, one hit, and the hit is `SETTEC` inside the Italian word **SETTECENTO** in `VILLE.EXE`. **Recorded reality 73.6533 %**, being 1 h 34 m 38 s read from declared headers with zero frames decoded — but **92.8 % of it belongs to the catalogue**, so on this object the column measures a retailer's advertising rather than the content of a work, and the row says so instead of printing the number bare. Leftovers **4,898,814 bytes, 1.0017 %**. **Sixty-two predictions written before the image was opened, 75.81 %**, method 78.57 % against content 75.45 %, with the likeliest miss flagged in advance and duly missed. And 104 files of readers' letters and 24,440 bytes of small ads carry **45 e-mail addresses and 73 telephone numbers**: those belonging to companies are quoted, those belonging to people are counted and appear nowhere in the repository |
| [**Il Mio Computer 02/2006**](https://github.com/vs-sr-dev/pc-ilmiocomputer0206-doc) | 2006, and for once the argument is short because **the packaging prints it**: `FEBBRAIO 2006` screen-printed on the disc face and repeated on the sleeve, against a primary volume descriptor reading `2006-01-04 16:46:00 GMT+01:00` — a February number cut on 4 January, the same relationship CLIC 02/97 had and the first time in this index it is **read rather than inferred**. *(The **issue number** is the interesting half and it is nowhere on the object: the volume identifier is `MICO02CD`, the sleeve says only the month, and no page prints a number. What dates it is the disc's own database of everything before it. `Micodb/` — which the briefing for that session read as an article archive and which is nothing of the kind — holds **684 records, one per program the magazine had published on a cover disc**, each naming its issue and month, and they run **113 (Dicembre 2004) to 128 (Gennaio 2006), sixteen consecutive numbers with no gaps** — thirteen a year, because a *Natale* issue sits between November and January. The next number is **129**, and it is derived, not printed. The same database says **102 of the 684 were *solo nel DVD***, which is why the sleeve has `CD` in a box in its corner)* | **Il Mio Computer**, published by **Future Media Italy S.p.A.** — and the attribution has two independent sources that agree, which the previous magazine row did not: the archive record names the publisher, and the disc carries three e-mail addresses on the `futuremediaitaly.it` domain. The full legal name appears **once** on the whole object, in the privacy notice at the foot of the questionnaire. Counted as literal strings over 810 text-shaped files, *Il Mio Computer* appears **696 times in 688 files** — but **686 of those are the masthead on the 685 database pages it copied in**, and in the front end a reader actually sees it names itself **nine times in two files**. It made **0.8111 %** of the bytes, 0.7509 % once ten Windows thumbnail caches and a Dreamweaver design note are taken out; CLIC 02/97, a different magazine from a different publisher nine years earlier, came out at 0.82 %. **It wrote no software at all**: `IERunner.exe`, the shell `autorun.inf` launches, is a Borland-built third-party utility — `Comments: TWebBrowser wrapper`, `LegalCopyright: (c) 2001-2002 Pavel Kilovatiy`, US-English translation, ISO record dated 2002 — which the session briefing had called the one thing the magazine produced |   | **Not a game — a software disc with four games on it, and the first object in this collection whose physical sectors survived.** 821,934,624 bytes, sha1 `c0818a7bbea879a29646ea57f0cbdb9b4a1473f3`, **349,462 sectors of 2,352 with no remainder**, mastered by `NERO BURNING ROM`. Every previous CD here was a copied tree, a cooked 2,048-byte image or a drive, and all three throw the same material away, so this is the first **full raw census** in the collection: 349,462 correct sync patterns, **349,462 header addresses equal to LBA + 150 in BCD**, 100 % Mode 2 Form 1 and **zero Form 2**, zero disagreeing subheader copies, and **349,462 of 349,462 EDCs verify** — computed column-wise across blocks of sectors because the inherited per-byte reader would need 718 million interpreted steps, and validated against that reader on 128 sectors before being trusted. **The frame costs 106,236,448 bytes, 12.9252 % of the file**, of which 97,849,360 is Reed-Solomon parity — **thirteen times everything the magazine wrote** — and no object here has ever been able to put a number on it. **CD-XA applies for the first time in fifteen objects**: `CD-XA001` at offset 1024 of *both* descriptors, and a 14-byte `XA` system-use area on **1,527 of 1,527 directory records**, every file number 0, every owner id 0, attributes `0x0D55`/`0x8D55` — 2,795,696 bytes of subheader carrying one bit of information repeated 349,462 times, *this is data*. **The two namespaces agree on everything except how to say it**: same 1,423 files, same 713,496,612 bytes, same dates, same extents, and **1,422 of 1,422 paths spelled differently** — the exact inverse of the previous disc, which had no Joliet at all. **100 % of the difference is explained by one mechanical ISO 9660 Level 1 rule** (uppercase, non-d-characters to `_`, truncate to 8.3, disambiguate with digits) with nothing unexplained; 29 groups collide, 115 names sit inside them and **exactly 86 carry a counter — the same 86 counted either way**, the worst group being sixteen weights of one typeface all truncating to `WORSTVEL.TTF`. **0 of 1,423 records violate Level 1** and the deepest is five levels down, so a DOS machine opens every file on the disc and **the front end would not work at all**, because the markup names its images in Joliet spelling. Extracted, the two trees hold **1,400 identical hashes each and none on one side only**. **The disc declares its own contents three times and the three disagree**: 34 printed bullets on the sleeve, **50 entries in its own HTML** each with a licence and an operating system, and **45 product directories**. The four numbers on the disc face — `28 PROGRAMMI`, `7 UTILITY`, `4 GIOCHI`, `5 PROGRAMMI COMPLETI` — turn out to be counts of **directories**, and **11 + 4 + 13 = 28** accounts for all 45 with nothing left over: the face counted the disc, the back listed the products, and the three the back omits (GTK, Java 2, Spybot) have full entries in the disc's own catalogue. **Of 31 printed version numbers 21 confirm and none is contradicted; of 13 Italian-language flags 6 confirm and none is contradicted** — the other seven are unverifiable because their products are single compressed installers, the same wall the recorded-reality column hits. And by the magazine's own catalogue **33 of 50 entries are freeware**, on a disc whose label says *da provare*. **67.8945 % of the object is 61 `.exe` files**; six files are 48.78 % of it; **755 HTML files are 0.2233 %**. Ninety-three binaries name **35 distinct companies**, at least twenty vendors, the most heterogeneous object here — and **16 of them carry a COFF timestamp of `1992-06-19 22:22:17`, all 16 and only 16 reporting linker 2.25**, which is Borland's constant and not a date. **Copy protection: none** — 12 markers over 99 executables, two hits, and both are InstallShield's `CompanyName` reading `Macrovision Corporation` because Macrovision bought InstallShield, which is a new class of false positive: not a word that collides, **a supplier that changed owners**. **Slack is 100.0000 % zero over 1,263,580 bytes, 0 of 1,422 files dirty**, which closes CLIC 02/97's Q5 against Toast — that disc had 2,820 of 2,821 files with a dirty tail and about 16.5 MB of a stranger's memory. **155 sectors at the end belong to nobody**, all zero, and they are the only run whose submode sets no bit and whose ECC field is blank — and the raw framing **cannot** say whether the burner or the dumper wrote them, because the EDC of an all-zero sector is zero and the parity of an all-zero codeword is zero. What settles it is a **four-byte integer at offset 80 of sector 16**: the volume space reads 349,462, the whole image, so Nero counted its own padding. On a disc where 106 million bytes exist to describe the data, the question was answered by four bytes describing the intention. **Recorded reality: 0.000000 %** — zero files, not a rounding, and the column is filled with the measurement and a footnote of equal length saying the method cannot see inside a compressor; the one compressed layer that *is* openable, 106 `.hmc` files of four-byte length plus RFC 1950 zlib, inflates **21.24×**, 1,851,712 bytes to 39,322,889. **646 absolute paths from 52 build roots, 568 of them from one product** — the only one that shipped unpacked — including a German developer's Sri Lanka holiday photographs used as template content; the same tool's **66 Macintosh-shaped hits are 0 paths**, being clock times, `Cc:` labels and an ATIP format string, reported separately and not summed. **Leftovers 8,645,394 bytes, 1.2117 %** — and they are a **complete record of the previous issue**, found three times by three tools: ten Windows thumbnail caches naming **154 images of which 24 are gone**, 100 install links of which **15 point at files that are not on the disc**, and the 684-record database that names those same missing products. **Zero shared bytes with any of the nineteen other objects here**, over 20 hash lists — the only two matches are the sha1 of the empty file. And the four games are **1.2664 %**, 9,035,493 bytes, four freeware board games of which three are bare executables under 600 KB: an Absolute Mastermind installer that is 82.55 % of the layer, a Connect Four and a draughts from Swedish personal home pages, and an **Othello written by a Greek teacher and his class in two lessons in 1997**, still carrying its `FILE_ID.DIZ` — a bulletin-board metadata format from the 1980s, pressed onto a newsstand disc in 2006. The fifth entry under `GIOCHI` is a manual for a game that is not here: *Grand Master Chess*, whose screenshot ships, whose four-page Italian manual ships, whose page calls it *completo*, and whose three rejected screenshots are still named by a thumbnail cache written the day before mastering. **Both of the magazine's own declarations transpose its name; the manual it shipped gets it right.** Three e-mail addresses on the publisher's domain, each appearing once: the subscriptions desk is quoted, and the two named employees whose working addresses were pressed onto the disc so readers would write to them are **counted and never written**, because on this object the name and the address are the same string. **Sixty-four predictions written before the image was opened, 71.09 %**, method 83.33 % against content 69.09 %, and **nine of the thirteen misses believed the session briefing instead of checking it** — while the one clause written against the briefing is the finding the second chapter rests on |
| [**Flight of the Amazon Queen**](https://github.com/vs-sr-dev/pc-flightoftheamazonqueen-doc) | **1995**, and the object carries **five** dates of which four are not the product's — the widest spread this column has had to argue since *Allods Online*, and unlike that one it is settled **inside the game data**. The credit roll exists **twice**: `DATA`, 5,172 bytes, reads `Copyright (c) **1994**, Interactive Binary Illusions`; `CREDIT1.CRD`, 1,540 bytes, is the same page with the same indents and reads `Copyright (c) **1995**`. Both ship on every copy. The 1994 roll credits a **Sound Engineer** and no recording infrastructure; the 1995 pair adds *Voice Recording Project Management*, *Executive Voice Producers*, two *Production Assistants* and a **Sound Mixer**, and corrects two people's names — `Will Jeffries` becomes `Will Jeffery`, `Mark Dearling` becomes `Mark Dearsley`. So the two years are not a contradiction: **1994 is the game and 1995 is the talkie**, and the same executable that indexes this file also indexes the 1994 floppy release at 22,677,657 bytes, one eighth the size, with the same rooms and no recordings. The cell takes the year of the object in hand, as the *Lands of Lore* row did with its 1993/94. *(The other three dates are in the object too and none is the product's: `Copyright (C) **1990** by Voyetra Technologies`, in a MIDI meta event inside `AQBANK.RL`, five years older than the game; `ScummVM 2.0.0 (Dec  9 **2017** 07:14:40)`, the interpreter's own `__DATE__`, and the only date the two largest binaries will give — both are GNU-linked with a COFF timestamp of zero, which is reproducibility refusing to answer; and **2018**-04-25 11:48:02 on all nine members of the shop's web cache. The filesystem says none of them: 41 of 45 mtimes fall inside **six seconds** of the morning this copy was installed, which is the *Lands of Lore* finding — 62 of 65 inside ten seconds — reproduced exactly. And the shortcut disagrees with the filesystem by precisely 7,200 seconds, because a Shell Link stores UTC)* | **Interactive Binary Illusions**, Brisbane — published by **Renegade, UK** — and the string was counted before the cell was filled, as this column now requires. `Interactive Binary Illusions` occurs **6 times in 190,787,021 bytes**, all six inside the three credit resources and nowhere else; `Renegade` 6 times, `Bitmap Brothers` twice, `John Passfield` 27, `Steve Stamatiadis` 23, `Tony Ball` 10. The studio is two people and the roll says which did what: story and design both, programming Passfield and Ball, graphics and animation Stamatiadis, script Passfield *(extra words by Steve)*. *(The credits also name the engine, which is why this row can do something the others cannot: **JASPAR Game Engine and other development systems — John Passfield**, and a separate line, **JASPAR PC Conversion — Tony Ball**. `.JAS` is JASPAR. That single line explains why a 1995 MS-DOS CD is indexed in **big-endian** and holds **375 AMOS memory banks** — AMOS being a BASIC for the Commodore Amiga — every one of them closing to the byte under a Motorola reading and to nonsense under an Intel one. The Amiga release came first, one person converted the engine, and nobody converted the data. Four PCX backgrounds are still **320 × 256**, which is a PAL Amiga screen)* |   | **A talking adventure whose talking is 89.2297 % of it, and whose script is 0.328242 %.** 45 files, 237,605,368 bytes; one of them, `queen.1`, is 190,787,021 and **contains no index of itself** — no header, no directory, no terminator, 7,671 resources laid end to end. The table that describes it is compiled into `scummvm.exe`, written by other people twenty-two years later, and **there are sixteen of them in there**, one per release the project knows. Six describe this file with zero holes and zero overlaps; five of the six are other countries' editions. The best-looking wrong one covers **99.9703 %** and leaves a 56,643-byte tail that reads exactly like a discovery — it is the German CD's table, **silently truncated** by a bounds check at the first record that would have run past the end of the English file. What separates them is not arithmetic but content: a `.PCX` begins with the byte `0A`, and on the winner **128 of 128** do while on every other table in the executable **none** do. Chosen that way, coverage closes at **100.000000 %**, 0 holes, 0 overlaps, 0 duplicate names, first byte 0 and last byte 190,787,021 — the most complete description of an object this collection has measured, and it was reconstructed by strangers. **Nothing on the disc is compressed**: 0 of 2,911 64 KiB blocks above 7.5 bits, and the commonest byte in the whole file is `0x80` at 11.37 %, which is silence in unsigned 8-bit PCM. The speech is **6,749 resources, 170,238,763 bytes, decoded to 4 h 16 m 13.662 s** — every container derived, zero failures, zero ambiguities — of which **32.74 minutes is the byte 128 exactly**, leader and trailer nobody trimmed because the CD was going to be 650 MB anyway. Against that, `QUEEN.JAS` — the whole logic — is **79,642 bytes, one part in 2,395.56**, and the entire written work including 27,575 words of script is 626,243. **Twelve** actors are credited with forty-three characters; the music is **259 Standard MIDI files** in five resources and contributes nought seconds. Delete the speech and the game still runs with subtitles; delete the 79,642 bytes and there is nothing left but a stranger's voice |
| [**Resident Evil**](https://github.com/vs-sr-dev/pc-residentevil-doc) | **1997**, and the object offers **five** dates of which four are not the product's. The primary volume descriptor reads `1997-08-22 12:13:04`, the newest file on the disc is `RESIDENTEVIL.EXE` at `1997-08-22 12:03:44` — and it is also the last file in extent order, at sector 312,222 — so the disc was closed on 22 August 1997 and the cell takes that year. *(The other four: the **game** this is a port of came out for the PlayStation in **1996**, and the executable still calls itself `bio1997` after the Japanese title while carrying the string `1997/3/21` in Japanese date order; the **oldest file** is `1995-07-10 21:09:00`, eight months before the PlayStation original shipped, and it is a Microsoft display driver for a Japanese PC, not a game asset — all 76 files dated 1995 are third-party; and the `.bin` file's own mtime says **1996-12-25 03:32:00**, which is **240 days before the volume it contains** and is therefore synthetic, the same impossibility `pecensus.py` tests binaries for, applied to an image and failing.)* **What makes this row different from every other one here is that the year is not the only thing the filesystem knows.** This is the first object in the collection whose directory records date the **work** rather than the copy: 1,102 distinct timestamps over **129 distinct days**, 1995-07-10 to 1997-08-22, proved two ways — the seconds field is even on **2,638 of 2,638** file records that are not exactly midnight, which is FAT and nothing else, while the 47 directory records written by the mastering program are even on **25 of 47**, which is chance; and four of the five root programs carry a recorded time exactly **nine hours** ahead of their own COFF linker stamp, to within fifteen seconds. Nine hours ahead of UTC is Japan. Every one of the 3,436 records declares a timezone byte of GMT+01:00, which is where the disc was mastered, and it is wrong about all of them | **Capcom Co., Ltd**, published in Europe by **Virgin Interactive Entertainment (Europe) Ltd** — and the strings were counted before the cell was filled. `Capcom` in any case occurs **21 times in 634,776,465 bytes** across seven files, `Virgin` **3 times**, and `Interactive Entertainment` **once**. Both attributions come from the same place: two lines of prose at the top of `README.TXT`. Everywhere else the name is a registry key, `Software\CAPCOM\RESIDENT EVIL`, in four of the five programs — and two of the three lower-case `virgin` hits are a path to `virgin.tim`, the publisher's logo screen. *(**Who did the port is not on the disc, and the silence is structural.** All five programs — `RESIDENTEVIL.EXE`, `LAUNCH.EXE`, `SETUP.EXE`, `UNINSTALL.EXE`, `CHECKER.EXE`, 2,163,200 bytes, one linker version, no Authenticode — carry **`VS_VERSIONINFO: ABSENT`**. Not one company name, not one product name, not one file version, on any binary the publisher wrote. The only path that leaks a machine at the company that made it is a single CodeView record, `E:` + `work` + `Unsetup` + `Release` + `Uninstall.pdb`, inside the uninstaller: **one leaked build path in 736,133,664 bytes**, against Final Fantasy VIII's 31,737. The mastering software names itself twice, in the two identification fields that are not blank — `GEAR VERSION 4.0 APRIL 1996` and `ELEKTROSON 1996`, both naming the company that wrote the burner — so the disc identifies its tool and neither of its authors)* |  | **A PlayStation game on a Windows CD, and the line between what was converted and what was not runs exactly along the boundary of what Microsoft had already written a reader for.** 736,133,664 bytes, sha1 `d664a7ad9203ac12ce400f63fb3bd03cf209a2db`, and it is **the disc**: 312,982 raw sectors of 2,352 in **Mode 1**, remainder zero, and the second full raw census in this collection — 312,982 correct sync patterns, 312,982 header addresses equal to LBA + 150, mode byte 1 on all of them, **312,982 of 312,982 EDCs verified and both Reed-Solomon parity blocks recomputed and matched on all 312,982**, zero mismatches of any kind. The inherited raw census could not do it: written for Mode 2 Form 1, it verifies its own arithmetic on 64 of 64 sectors and matches the disc on **0 of 64**, because Mode 1 computes its error-detection code over bytes 0..2063 and Mode 2 Form 1 over 16..2071. Same 304 bytes of frame in both modes, so **12.9252 % is not a fact about either disc — it is 304 ÷ 2352** and this repository says so instead of reporting a coincidence. Inside is one ISO 9660 volume with **no Joliet at all** — one namespace, and the game's own executable, `RESIDENTEVIL.EXE`, has a name that namespace read strictly cannot spell, one of eight that break 8.3. **The port:** every recording of the world is in a PC format and every description of the world is in the PlayStation's. **2,601 TIM textures and 828 TMD models** validated inside a Win32 product — the raw identifier grep finds 44,047 candidates and the format's own length arithmetic kills twelve of every thirteen — with **zero bytes of film or speech in a PlayStation container and zero textures or models in a RIFF one**. `.IVM` turns out to be a TIM followed by a TMD, a texture and the model it wraps, 77 of 77 closing on the byte; `.ETM` is eight TIMs and a trailing index of eight offsets written in **descending** order; the TMD pointer base was settled by arithmetic on one file, where one reading leaves the normals 28 bytes past the end and the other lands the last normal on the last byte. **`.PAK` was derived from the bytes before any description of it was in hand** — entropy 7.95, no repeated four-byte sequence in 42,634, sizes ending mid-byte, and a tail with a period of exactly forty bits incrementing by `0x0040100401`, which is four ten-bit fields each one greater than the last, and first nine bits reading **16**, which is `10 00 00 00`, which is a TIM identifier sitting in the open before a single dictionary entry exists. It is LZW over a TIM: **1,112 of 1,112 decompress and close with residue zero**, 38,754,842 bytes becoming 124,641,776. And then it was **looked at**, which arithmetic cannot do: **28 of the 676 backgrounds are stored four pixels narrower than their own header declares** — 26 of them in four contiguous rooms of `STAGE3` — measured as a modal row-to-row shift of +4 on up to 233 of 239 row pairs, with the missing four columns held back and written after the last row. Nothing in those files distinguishes them from the 643 that are right. **Recorded reality is two things at once for the first time here** and the repository publishes both rather than choosing: 27 Cinepak `.AVI` at 320×240, 12,598 frames declared and **12,598 counted in the movie lists**, **1,199.7988 s** at two frame rates, **38.6910 %** of the file bytes; and 1,110 PCM `.WAV`, **5,027.658866213 s**, closed by two independent routes with a largest disagreement of **0.000000000 s** and a container that accounts for every byte of 1,110 files of 1,110, of which the dialogue is **16.9618 %**. Together **68.5388 % of the file bytes and 59.1018 % of the raw image** — and the 9.4370 points between those two numbers are the CD frame, which no previous object in this index could put inside the fraction. **The film is also the only thing the installer refuses to copy**: `SETUP.EXE` names 2,895 distinct files, and set against the disc the set it never mentions is **exactly the 27 `.AVI`** and the set it names that is not there is **empty**. The music is eight-bit and the speech is sixteen. **Thirteen cross-disc hits, after eight sessions of zero** — a DOS/4GW extender byte-identical to the copy inside a Tomb Raider demo on an Italian magazine disc from five months earlier, compared 265,396 bytes at a time and not by hash, and twelve DirectX driver files identical to Final Fantasy VIII's, two of them **under different names** — and not one byte of game data among them, which turns *every disc is an island* into *every disc is an island except for the software nobody on it wrote*. **Copy protection: none**, 11 schemes, 0 hits, positive control firing on 370 files. **Slack 100.0000 % zero over 3,599,471 bytes, 0 dirty files of 3,436**, and 1,144 unclaimed sectors in 31 runs with **zero runs containing anything** — no unintentional leftover at all, on a disc whose deliberate leftovers come to 11,533,102 bytes, 1.8169 %, and include **4,137,303 bytes of drivers for the NEC PC-9800**, a computer sold only in Japan and incapable of running the game. And **the price of not being a console is 3.7889 %**: DirectX 3a as a prerequisite the operating system genuinely lacked, three vendors' development drivers the readme admits to in writing, six command-line switches named after six accelerator cards, and a `LAUNCH.EXE` that is **66.82 % the size of the game** and exists only to work out what is in the machine. **62 predictions written before the object was opened: 36 hit, 17 half, 9 miss, 71.8 %** — and six of the nine misses believed the session briefing's description of a file header instead of opening the file |
